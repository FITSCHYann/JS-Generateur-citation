...
let nombeAleatoire = null;

let auteur = document.querySelector("#auteur");
let citation = document.querySelector("#citation");

citation.textContent = citations[0][0];

let btn = document.querySelector("#nouveau");
btn.addEventListener("click", nbAleatoire);

function nbAleatoire() {
  let dernierNombre = nombeAleatoire;

  do {
    nombeAleatoire = Math.floor(Math.random() * (9 - 0 + 1)) + 0;
  } while (nombeAleatoire === dernierNombre);

  citation.textContent = citations[nombeAleatoire][0];
  auteur.textContent = citations[nombeAleatoire][1];
}
